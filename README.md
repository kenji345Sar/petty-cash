# Petty Cash（店舗現金管理システム）

業務経験をもとに、個人で設計・実装した学習用プロジェクトです。

## 扱う2つの出納

店舗ごとの金庫を、性質の違う2つの帳簿で管理する。金庫という置き場所を共有しているだけで、業務としては別物。

| 帳簿 | 業務 | 画面 |
|---|---|---|
| 小口現金 | 事務用品・交通費などの経費の立替と精算 | 小口タブ |
| 売上金 | レジの釣銭準備と、売上の回収・業者（警備会社・銀行）への引き渡し | 売上金タブ |

リポジトリ名と C# の名前空間は、作り始めた経緯から `petty-cash` / `Vendor` のままになっている。画面と仕様書の呼び名が実態に合うほうを採用している（対応は [docs/spec/README.md](docs/spec/README.md) の用語を参照）。
...
| **ドメイン** | **PettyCash.Domain** | **業務ルールそのもの（金庫・小口・売上金）** |
