...
import type { CashBag, PrepBag, DenominationCheck, Denomination } from "../api/client";
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div>
              <label>備考</label>
              <input type="text" value={description} onChange={(e) => setDescription(e.target.value)} style={{ marginLeft: 8, width: 300 }} placeholder="例: 売上入金" />
            </div>
            <div>
              <label>日付</label>
              ...
              return (
                <Fragment key={bag.id}>
                  <tr style={selected.has(bag.id) ? { background: "#e8f4fd" } : undefined}>
                    <td>BAG-{String(bag.id).padStart(3, "0")}</td>
