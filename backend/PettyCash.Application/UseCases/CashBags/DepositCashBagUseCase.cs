...
public class DepositCashBagUseCase(ICashBagRepository cashBagRepository, ISequenceNumberService sequenceNumberService, IBalanceService balanceService, IProjectionService projectionService, IEventStore eventStore, IUnitOfWork unitOfWork)
{
    public async Task<CashBagDto> ExecuteAsync(DepositRequestDto dto)
    {
        ...
        await balanceService.AssignBalanceAsync(bag.Transaction!);
        await cashBagRepository.AddAsync(bag);
        await eventStore.AppendAsync(bag.Transaction!);
        await projectionService.ProjectAsync(bag.Transaction!);
