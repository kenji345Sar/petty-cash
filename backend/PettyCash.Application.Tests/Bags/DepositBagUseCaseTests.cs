...
public class DepositBagUseCaseTests
{
    ...
    private DepositBagUseCase CreateUseCase() =>
    ...
    public async Task 金種指定で正常作成されAddとSaveが呼ばれる()
    {
        var dto = new DepositRequestDto(
            ...
            Denomination: new DenominationDto(1, 0, 0, 0, 0, 0, 0, 0, 0)); // 10000

        var result = await CreateUseCase().ExecuteAsync(dto);

        Assert.Equal(10000, result.TotalAmount);
        Assert.Equal("InSafe", result.Status);
        _bagRepo.Verify(r => r.AddAsync(It.IsAny<ChangeBag>()), Times.Once);
    ...
    public async Task 金種なしで例外でAddは呼ばれない()
    {
        ...
        _bagRepo.Verify(r => r.AddAsync(It.IsAny<ChangeBag>()), Times.Never);
