...
## 業者 — 釣り銭バッグ（ChangeBags）

| メソッド | エンドポイント | 用途 |
|---|---|---|
| GET | `/api/bags?safeId=` | 釣り銭バッグ一覧 |
| POST | `/api/bags/deposit` | 釣り銭バッグ入金 |
| POST | `/api/bags/{id}/move` | レジへ移動 |

## 業者 — キャッシュバッグ（CashBags）

| メソッド | エンドポイント | 用途 |
|---|---|---|
| GET | `/api/cashbags?safeId=` | キャッシュバッグ一覧 |
| POST | `/api/cashbags/deposit` | キャッシュバッグ入金 |
...
| POST | `/api/denominationchecks/changebag/{bagId}` | 釣り銭バッグの有高チェック |
| POST | `/api/denominationchecks/cashbag/{bagId}` | キャッシュバッグの有高チェック |
