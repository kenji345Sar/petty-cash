...
業者から受け取った釣り銭をひとまとめにした袋。金庫で保管し、必要に応じてレジへ移動する。
...
- 1つ以上のキャッシュバッグが必要
- 既に別の準備バッグに含まれているキャッシュバッグは含められない（`PrepBagId != null` チェック）
- 合計金額はキャッシュバッグの合計から自動算出
- 引渡時に出金取引（VendorTransaction Withdrawal）が自動生成される
- キャンセル後のキャッシュバッグは再割当可能になる（`CashBags` が Clear される）
