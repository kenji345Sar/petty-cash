...
> 金庫（店舗）の売上金（レジの釣銭準備・売上の回収）を、バッグで管理しながら出納を記録する画面。
> 業務としては小口現金とは別物（→ [README.md](./README.md) の「2つの出納」）。コード上の名前は `Vendor` 系。
...
<!-- 期間ナビ / バッグ管理（折りたたみ）/ 売上金出納帳テーブル / 各モーダル。
     バッグ管理は既定で開いている。 -->

## 2. バッグ管理

売上金側の現金は3種類のバッグで管理する。各バッグの操作は売上金出納帳の取引として自動記録される。
...
- 金額は金種の合計から自動算出される
- レジへの移動は1回のみ（二重移動は例外）
- 入金時・移動時にそれぞれ売上金取引（VendorTransaction）が自動生成される
...
- 入金時に売上金取引（VendorTransaction Deposit）が自動生成される
...
<!-- バッグ種別ごとにチェック（change/cash/prep）。expectedAmount の取り方が種別で異なる。
     詳細ロジックは common.md、ここは売上金タブ固有の点。 -->

## 5. 赤伝（修正）

赤伝の基本（逆転ルール・追記型・残高の積み方）は [common.md](./02-common.md#3-赤伝逆仕訳) を参照。ここでは売上金タブ固有の制約を記す。

### 現状の挙動と制約

売上金の出納帳取引は**すべてバッグ操作の副産物**（`MoveBagToRegister` / `MoveCashBagToSafe` が生成）であり、直接の入出金登録は存在しない。
...
バッグ移動の取消（undo）を新設し、**バッグ状態と出納帳取引をセットで戻す**ように寄せる。これが入ったら売上金出納帳の赤伝ボタンは廃止し、修正はバッグ操作の取消に一本化する。
...
売上金の赤伝（入金取引を打ち消す＝出金になる）は、**売上金残高の範囲内**に限る。同じ金庫に小口現金があっても、売上金の出金には使えない。

```
残高不足です。売上金残高: 48,000円、出金額: 50,100円
```

実装: `Safe.EnsureCanWithdrawVendor()`

一方、**バッグの操作（両替金バッグのレジ移動・準備バッグの引渡）には残高チェックがない**。バッグに入っている金額しか動かないため、帳簿上は残高を超えない前提になっている。
...
<!-- getVendorDashboard / depositBag / moveBagToRegister / depositCashBag /
     ...
     api-screens/api-endpoints.md から売上金分を抜粋。 -->
