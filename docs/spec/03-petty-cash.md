...
逆転ルール・追記型・残高の積み方は [02-common.md](./02-common.md#3-赤伝逆仕訳) を参照。
...
小口の出金は、**小口残高の範囲内**に限る。同じ金庫に売上金があっても、小口の出金には使えない。小口現金と売上金は別の出納だからである。

| 操作 | 判定に使う残高 |
|---|---|
| 入出金登録の「出金」 | 小口残高 |
| 入金取引の赤伝（＝出金になる） | 小口残高 |
| 出金取引の赤伝（＝入金になる） | 判定しない（残高は増えるため） |

超える場合はエラーになり、取引は登録されない。

```
残高不足です。小口残高: 2,000円、出金額: 3,000円
```

実装: `Safe.EnsureCanWithdrawPettyCash()`（[architecture/02-domain-model.md](../architecture/02-domain-model.md)）

### その他

- 出金額は1円以上。0円以下は登録できない。
- 有高チェックの帳簿額も小口残高を使う（→ 3.）。売上金は含めない。
- 過去日付の扱いは未確定（[issue.md](../issue.md) の 4.）。
